...
- **Plan 2 — Ports from personal repo:** pi harness reconciliation (best-of-both diff), `hermes-selflearning` + `self-customize`, `agents-compose` size guard, `fetch_url_to_workspace`, Apple Container orphan-cleanup fix (dual-shape status, personal `47b8be1` — this repo's `container-runtime.ts:168` still string-matches); then Apple Container upgrade 0.12.3 → 1.1.0 STRICTLY AFTER that fix lands (`brew upgrade container`, restart container system + BOTH installs' services in one window — shared binary/daemon with the personal install — verify one spawn each). Also: `composeGroupClaudeMd`-after-`ensureRuntimeFields` container.json clobber ordering bug, dead `'claude'` provider fallback (`container-runner.ts:~260`), non-fatal readonly-DB follow-up-poll error.
