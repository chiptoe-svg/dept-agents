...
- **No document conversion in A3 — deferred to a later phase as skills.** A3 gets the *file* to the agent; it does NOT convert Office formats. What the agent can read today: **PDF** via the existing `pdf-reader` container skill (`pdftotext`), **images** inline (vision). **Office files (`.pptx`/`.docx`/`.xlsx`) land raw** in the agent's workspace — usable best-effort but not richly parsed. Rich document intelligence — host-side **docling** text extraction and the **picture-description** pipeline (per-slide vision descriptions via the DGX `gemma-4-26b`, reusing the curriculum project's `docling-serve`) — is explicitly a **future phase, delivered as installable skills** (small-trunk-with-skills philosophy: external-service document parsing belongs in a skill, not trunk). Not in A3.
...
- **Document conversion / intelligence** (docling text extraction, the picture-description/DGX-vision workflow, markitdown, LibreOffice) — deferred to a later phase and delivered as installable **skills**, not built into A3. A3 attaches files; reading Office formats richly is future work.
